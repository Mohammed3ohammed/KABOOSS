...
const Navbar = () => {
  ...
  return (
    <nav className="z-50">
      ...
      <h1 className="text-red-800 italic text-3xl"><span className="text-black">kABOOSS</span> GYM</h1>
        ...
        <ul id="mobile-menu" className={`${isMenuOpen ? "block" : "hidden"} absolute md:static top-20 left-0 w-full flex flex-col md:flex-row items-center md:w-auto bg-white md:bg-transparent md:flex text-black gap-5 lg:gap-10 p-4 md:p-0 abs z-50 text-right shadow-lg md:shadow-none`}>
          <li className="py-1 md:py-0 mt-1 sm:mt-0 z-50 transition hover:text-red-800">
          ...
          <Link href="/log" prefetch={true} className="md:ml-10 border-2 border-solid border-red-800 p-1 text-center w-32 rounded-full transition text-red-800 hover:bg-red-800 hover:text-white" onClick={() => setMenuOpen(false)}>{t("login")}</Link>
          <Link href="/sign" prefetch={true} className="border-2 border-solid border-red-800 p-1 text-center w-32 rounded-full transition hover:bg-red-800 hover:text-white" onClick={() => setMenuOpen(false)}>{t("signup")}</Link>

          <Link href="/log" prefetch={true}>
            <FontAwesomeIcon
              icon={faUser}
            className="w-6 h-6 text-red-800 transform transition duration-300 hover:scale-150 animate-bounceLight"
