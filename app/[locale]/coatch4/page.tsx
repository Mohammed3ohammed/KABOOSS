...
import logo from "../../../public/Captins/Image5.jpg";
...
const Page = () => {
  const t = useTranslations("Coaths");
  ...
  return (
    <>
      <div className='flex justify-center'>
        <h1 className='text-3xl font-bold mt-10 italic text-red-800'>{t("coatch4.title")}</h1>
      </div>
      <div>
        <p className='text-center mt-5 text-2xl'>{t("coatch4.result")}</p>
          ...
          <div>
            <section className="py-12 px-6 lg:px-20">
              <div className="max-w-4xl mx-auto p-8 shadow-lg rounded-2xl">
                <h2 className="text-3xl font-bold text-gray-400 mb-6 text-center">{t("coatch4.name")}</h2>
                <p className="text-gray-700 text-lg mb-4">{t("coatch4.description")}</p>

                <div className="mt-6">
                  <h3 className="text-2xl font-semibold text-gray-400 mb-3">{t("coatch4.customized")}</h3>
                  <ul className="list-disc list-inside text-gray-400 space-y-2">
                    <li>{t("coatch4.weight")}</li>
                    <li>{t("coatch4.muscle")}</li>
                    <li>{t("coatch4.fitness")}</li>
                  </ul>
                </div>

                <div className="mt-6">
                  <h3 className="text-2xl font-semibold text-gray-400 mb-3">{t("coatch4.holistic")}</h3>
                  <ul className="list-disc list-inside text-gray-400 space-y-2">
                    <li>{t("coatch4.latest")}</li>
                    <li>{t("coatch4.nutrition")}</li>
                    <li>{t("coatch4.energy")}</li>
                  </ul>
                  <form onSubmit={handleSubscribe} className='flex flex-col gap-5 lg:w-8/12 p-4 md:w-8/12 w-full justify-center'>
                    ...
                    >
                      <option value="">{t("coatch4.gym_plan")}</option>
                      <option value="monthly">{t("coatch4.monthly")}</option>
                      <option value="quarterly">{t("coatch4.quarterly")}</option>
                    </select>
                    <button type="submit" className="border-2 border-solid border-red-800 py-2 px-6 mt-5 rounded-full text-white hover:bg-orange-500 hover:text-white transition duration-300 w-full sm:w-auto text-center cursor-pointer">
                      {t("button")}
                    </button>
                  </form>
                </div>
